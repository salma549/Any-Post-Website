...
const HealthLists = () => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/freelance/${formData.category}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/freelance/${formData.category}`, data);
