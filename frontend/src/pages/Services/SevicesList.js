...
const VechilesList = () => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/services/${formData.category}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/services/${formData.category}`, data);
