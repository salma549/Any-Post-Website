...
const VechilesList = () => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/electronics/${formData.category}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/electronics/${formData.category}`, data);
    ...
    <>
      <form onSubmit={handleSubmit} className="post-form">
        ...
        <option value="create3">All</option>
          ...
          <option value="tvs-audio">TVs, Audio, Video</option>
